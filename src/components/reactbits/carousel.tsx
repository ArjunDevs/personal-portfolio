...
import { observeInView } from '../../lib/motion';
...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
  ...
  return (
    <motion.div
      ...
      className={`relative shrink-0 flex flex-col ${
        round
          ...
          : 'items-stretch justify-start bg-white/3 border border-white/15 rounded-xl'
    ...
    >
      {item.image ? (
        <>
          <div className="flex w-full flex-1 items-center justify-center overflow-hidden bg-black">
            <img
              src={item.image}
...
export default function Carousel({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    return observeInView(el, setIsInView);
  }, []);
  ...
  }, [autoplay, autoplayDelay, isHovered, pauseOnHover, isInView, itemsForRender.length]);
  ...
  return (
    <div
      ...
      className={`relative flex flex-col overflow-hidden p-4 ${
        round ? 'rounded-full border border-white' : 'h-full rounded-3xl border border-white/15'
      }`}
      style={{
        width: `${baseWidth}px`,

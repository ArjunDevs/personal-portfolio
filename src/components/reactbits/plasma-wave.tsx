...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    // The shader raymarches every pixel each frame, so it is only worth running
    // while the Hero is actually on-screen. Off-screen (scrolled past, or tab
    // hidden) it is paused to free the GPU/main thread for the lower sections.
    const start = () => {
      if (running) return;
      running = true;
      animateId = requestAnimationFrame(update);
    ...
    start();
    const stopObserving = observeInView(ctn, (inView) => (inView ? start() : stop()));
